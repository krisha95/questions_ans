...
import { BiCloudDownload, BiInfoCircle, BiToggleLeft } from "react-icons/bi"
import { BsCardText, BsGear, BsLifePreserver, BsPerson, BsPower } from "react-icons/bs"
import { FaSearch } from "react-icons/fa"

const demoMenus = [
    { label: "Home q&a v1", link: "/que-ans/index" },
    { label: "Home q&a v2", link: "/que-ans/index-v2" },
    { label: "Home q&a v3", link: "/que-ans/index-v3" },
    { label: "Home Forum", link: "/forum/index-forum", active: true },
    { label: "Home Help-center", link: "/help-center/index" },
    { label: "Home Help-center v2", link: "/help-center/index-v2" },
    { label: "Home Ticket", link: "/tickets/index-ticket" },
    { label: "Home Discussion", link: "/index-discussion" },
]

const extraLinks = [
    { label: "RTL demo", link: "https://q&a.webestica.com/rtl/", icon: <BiToggleLeft /> },
    { label: "Support", link: "https://support.webestica.com/", icon: <BsLifePreserver /> },
    { label: "Documentation", link: "docs/index.html", icon: <BsCardText /> },
    { label: "Buy Q&A!", link: "https://themes.getbootstrap.com/store/webestica/", icon: <BiCloudDownload className="text-success" /> },
]

const mainLinks = [
    { label: "Forum Category", link: "/forum/forum-category" },
    { label: "Detail", link: "/forum/forum-detail" }
]
        ...
        <header className="header-sticky header-absolute bg-mode">
            <nav className="navbar navbar-expand-xl">
                <div className="container">
                    <Link className="navbar-brand me-5" href="index.html">
                        <Image className="light-mode-item navbar-brand-item" src={logo} alt="logo" width={120} />
                        <Image className="dark-mode-item navbar-brand-item" src={logoLight} alt="logo" width={120} />
                    </Link>
                    ...
                    <div className={`navbar-collapse collapse ${openMenu ? "show" : ""}`} id="navbarCollapse">
                        <ul className="navbar-nav navbar-nav-scroll dropdown-hover">
                            <li className="nav-item dropdown">
                                <Link className="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown">Demos</Link>
                                <ul className="dropdown-menu">
                                    {demoMenus.map((item, i) => (
                                        <li key={i}>
                                            <Link href={item.link} className={`dropdown-item ${item.active ? "show" : ""}`}>{item.label}</Link>
                                        </li>
                                    ))}
                                    <li className="dropdown-divider"></li>
                                    {extraLinks.map((item, i) => (
                                        <li key={i}>
                                            <Link href={item.link} className="dropdown-item" target="_blank" rel="noopener noreferrer">
